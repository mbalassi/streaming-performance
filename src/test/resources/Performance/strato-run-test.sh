...
if [ -d "${toDir}" ] ; then
	...
	ssh -n $stratoUser@$stratoMaster "timeout ${length} ./$stratoDir/bin/flink run -v -j ./$stratoDir/lib/${jarFile} -c $classPath -a cluster $resourcePathOnCluster ./${stratoDir}/log/counter/ ./${stratoDir}/lib/${jarFile} ${stratoHost} ${stratoRcpPort} ${paramsWithSpace}"
